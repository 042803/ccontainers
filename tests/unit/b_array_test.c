...
void test_init_from_array(void) {
    ...
    struct Array arr = array_init_from(values, len);
...
void test_push_back(void) {
    ...
    CU_ASSERT_EQUAL(array_get(&arr, 1), 99);
...
void test_get(void) {
    ...
    CU_ASSERT_EQUAL(array_get(&arr, 0), 10);
    CU_ASSERT_EQUAL(array_get(&arr, 1), 20);
    CU_ASSERT_EQUAL(array_get(&arr, 999), -1); 
    free_arr(&arr);
}
...
void test_set(void) {
    ...
    CU_ASSERT_EQUAL(array_get(&arr, 0), 100);
    CU_ASSERT_EQUAL(array_get(&arr, 1), 200);

    array_set(&arr, 999, 300);
    CU_ASSERT_EQUAL(arr.length, 2);
    free_arr(&arr);
}
...
int main(void) {
    ...
    CU_add_test(suite, "array_init", test_init);
    CU_add_test(suite, "array_init_from", test_init_from_array);
    CU_add_test(suite, "push_back", test_push_back);
    CU_add_test(suite, "array_get", test_get);
    CU_add_test(suite, "array_set", test_set);
