...
void test_set_to_range(void) {
    // Normal range
    int vals1[] = {1, 2, 3, 4, 5};
    struct Array arr1 = array_init_from(vals1, 5);
    ...
    struct Array arr2 = array_init_from(vals1, 5);
    ...
    /*
    struct Array arr3 = array_init_from(vals1, 5);
...
void test_apply_to_range(void) {
    ...
    // NULL function
//    struct Array arr2 = array_init_from(vals1, 4);
//    apply_to_range(&arr2, 0, 2, NULL); // No change
//    CU_ASSERT_EQUAL(arr2.A[0], 1);
//    free_arr(&arr2);

    // Invalid range
    struct Array arr3 = array_init_from(vals1, 4);
...
//    struct Array arr2 = array_init_from(vals1, 3);
